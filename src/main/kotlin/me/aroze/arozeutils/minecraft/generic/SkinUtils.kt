...
fun validateTextureURL(url: String): Boolean {
    ...
    return if (!"(?:https?:\\/\\/)?textures\\.minecraft\\.net\\/texture\\/(.+)".toRegex().matches(url)) false
    else (validateTextureFromApi(url))
...
fun validateTextureBase64(textureString: String, checkTexturesApi: Boolean): Boolean {
    ...
    try {
        b64 = String(Base64.getDecoder().decode(textureString)).replace("\"", "")
    } catch (e: Exception) {
        return false
    }
...
private fun stripTextureForApi(texture: String): String {
    return texture
        .replace("\"", "")
        .replace("https", "http")
